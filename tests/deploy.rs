...
use dot::{deploy, manifest::Manifest};
...
impl Trees {
    ...
    fn deploy(&self, manifest: &Manifest) -> anyhow::Result<()> {
        self.deploy_with(manifest, &mut HashMap::new())
    }
    ...
    fn deploy_with(
        ...
        deploy::tree(&self.src, &self.dst, manifest, local_variable_map, false)
    }
}

fn targets(entries: &[(&str, Target)]) -> Manifest {
    Manifest {
        targets: entries
            .iter()
            .map(|(path, target)| (Path::new(path).to_path_buf(), *target))
            .collect(),
    }
}
...
fn an_extension_under_the_nuke_one_names_the_target() {
    ...
    trees.deploy(&Manifest::default()).expect("a deployment");
...
fn a_table_names_the_target_of_a_file_that_is_named_rather_than_extended() {
    ...
    let manifest = targets(&[(".config/ghostty/config", Target::Ghostty)]);

    trees.deploy(&manifest).expect("a deployment");
...
fn a_template_still_renders_beside_a_document() {
    ...
    trees
        .deploy_with(&Manifest::default(), &mut local_variable_map)
        .expect("a deployment");
